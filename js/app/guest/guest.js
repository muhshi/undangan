...
export const guest = (() => {
   ...
   */
  const pageLoaded = () => {
    const hasSelfApi = !!document.body.dataset.url;
    lang.init();
    offline.init();
    if (!hasSelfApi) {
      comment.init();
    ...
    if (!token || token.length <= 0) {
      if (!hasSelfApi) {
        // MODE DEMO TANPA data-key → tetap hapus komentar
    ...
    if (token && token.length > 0) {
      if (hasSelfApi) {
        // SELF-HOST MODE: JANGAN panggil session.guest (API demo).
        ...
        // DEMO MODE (pakai data-key) → jalankan bawaan
        progress.add();
        progress.add();
